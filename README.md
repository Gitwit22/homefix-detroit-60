...
People assisting with an intake are stored as case contacts, never as residents. A case contact may be authorized without becoming primary; when made primary, the intake requires permission acknowledgment and records its timestamp. Existing cases are not backfilled: when a case has no primary assistant, resident contact information remains the communication and inspection-scheduling fallback.
